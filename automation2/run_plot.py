...
def get_paths(name: str, format: str = None):
    ...
    # format = .jpeg
    if name == "bmp":
        files = glob.glob('automation2/'+ name + '/*.bmp')
    else:
        files = glob.glob('automation2/'+ name + '/*' + format.lower())
    n = len(files)
    paths = []
    for i in range (n):
        if name == "bmp":
            paths.append("automation2/" + name + "/mri_" + str(i+1) + ".bmp")
        else:
...
excel_file_path = 'automation2/result.xlsx'
...
plt.tight_layout()
plt.savefig('automation2/plot/psnr.png')
...
plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
plt.tight_layout()
plt.savefig('automation2/plot/ssim.png')
...
plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
plt.tight_layout()
plt.savefig('automation2/plot/size.png')
...
plt.tight_layout()
plt.savefig('automation2/plot/psnr_size.png')
...
plt.grid(which='minor', linestyle=':', linewidth='0.5', color='gray')
plt.tight_layout()  
plt.savefig('automation2/plot/ssim_size.png')