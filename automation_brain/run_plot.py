...
file_path = 'automation_brain/mri_brain.dcm'
...
for i in range(pixel.shape[0]):
    ...
    # im.save("automation_brain/bmp/mri_" + str(i+1) + ".bmp", 'BMP')
    bmp_folder = 'automation_brain/bmp'
    os.makedirs(bmp_folder, exist_ok=True)
    bmp_filename = bmp_folder + '/mri_' + str(i+1) + '.bmp'
    cv2.imwrite(bmp_filename, final_img)
    for x in ext:
        for y in quality:
            folder_name = "automation_brain/" + x[1:].lower() + "_" + str(y)
...
def get_paths(name: str, format: str = None):
    ...
    # format = .jpeg
    if name == "bmp":
        files = glob.glob('automation_brain/'+ name + '/*.bmp')
    else:
        files = glob.glob('automation_brain/'+ name + '/*' + format.lower())
    n = len(files)
    paths = []
    for i in range (n):
        if name == "bmp":
            paths.append("automation_brain/" + name + "/mri_" + str(i+1) + ".bmp")
        else:
            paths.append("automation_brain/" + name + "/mri_" + str(i+1) + format.lower())
    return paths
...
excel_file_path = 'automation_brain/result.xlsx'
...
plt.tight_layout()
plt.savefig('automation_brain/plot/psnr.png')
...
plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
plt.tight_layout()
plt.savefig('automation_brain/plot/ssim.png')
...
plt.legend(bbox_to_anchor=(1.0, 1.0), loc='upper left')
plt.tight_layout()
plt.savefig('automation_brain/plot/size.png')
...
plt.tight_layout()
plt.savefig('automation_brain/plot/psnr_size.png')
...
plt.grid(which='minor', linestyle=':', linewidth='0.5', color='gray')
plt.tight_layout()  
plt.savefig('automation_brain/plot/ssim_size.png')